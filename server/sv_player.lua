...
function loadUser(identifier, source, new, licenseNotRequired)
	...
	db.retrieveUser(identifier, function(user, isJson)
		...
		if user.license or licenseNotRequired then
			...
			TriggerEvent('redem:playerLoaded', Source, Users[Source])
			...
			if new then
				TriggerEvent('redem:newPlayerLoaded', Source, Users[Source])
            end
		else
			-- Irrelevant
			local license
...
AddEventHandler('redem:getPlayers', function(cb)
	cb(Users)
end)
...
AddEventHandler("redem:setPlayerData", function(user, k, v, cb)
	if(Users[user])then
...
AddEventHandler("redem:setPlayerDataId", function(user, k, v, cb)
...
AddEventHandler("redem:getPlayerFromId", function(user, cb)
	if(Users)then
		if(Users[user])then
			cb(Users[user])
		else
